...
{
	if (qx==NULL) throw RC_NOMEM; qx->ref();
	qx->vals[QV_PARAMS]=ectx!=NULL&&ectx->params!=NULL&&ectx->nParams!=0?ectx->params[0]:Values(NULL,0); qx->vals[QV_PARAMS].fFree=false;
...
RC QBuildCtx::process(QueryOp *&qop)
{
	...
	case QVF_DISTINCT:
		if ((qop->qflags&QO_UNIQUE)==0 && (qv->stype==SEL_PINSET||qv->stype==SEL_COMPOUND||qv->stype==SEL_AUGMENTED)) {
			...
			if (q==NULL) {delete qop; qop=NULL; return RC_NOMEM;}
			qop=q;
		}
		break;
#if 0
	case DT_DISTINCT_VALUES:
		if (propsReq.nPls!=0 || (qop->qflags&QO_UNIQUE)==0) {
			...
			if (os==NULL) {delete qop; qop=NULL; return RC_NOMEM;}
			...
			if (q!=NULL) qop=q; else {delete qop; qop=NULL; return RC_NOMEM;}
		}
		break;
#endif
	case QVF_ALL:
		...
		if (ce->propID1==PROP_SPEC_PINID && ce->propID2==PROP_SPEC_PINID) {
			...
			if (qctx.sortReq!=NULL && qctx.nSortReq!=0) {
				...
				if (QBuildCtx::checkSort(qctx.src[nqs0],qctx.sortReq,qctx.nSortReq,nP)) {
					if ((q=new(qctx.ses) HashOp(qctx.src[nqs0],qctx.src[nqs0+1]))==NULL) rc=RC_NOMEM;
					break;
				}
				if (QBuildCtx::checkSort(qctx.src[nqs0+1],qctx.sortReq,qctx.nSortReq,nP)) {
					if ((q=new(qctx.ses) HashOp(qctx.src[nqs0+1],qctx.src[nqs0]))==NULL) rc=RC_NOMEM;
					break;
				}
			}
...
// merge propsReq with getProps
	...
	for (unsigned i=0; rc==RC_OK && i<nClasses; i++) {
		...
		if (qctx.nqs>=sizeof(qctx.src)/sizeof(qctx.src[0])) rc=RC_NOMEM;
			...
			if (qctx.ncqs>=sizeof(qctx.condQs)/sizeof(qctx.condQs[0])) rc=RC_NOMEM;
				...
				if ((qs.qry=cqry->clone(STMT_QUERY,qctx.ses))==NULL) rc=RC_NOMEM; else {qs.params=(Value*)cs.params; qs.nParams=cs.nParams;}
			}
		} else if (cidx==NULL) {
			if ((qctx.src[qctx.nqs++]=new(qctx.ses) ClassScan(qctx.qx,cid,qctx.flg))==NULL) rc=RC_NOMEM;
			...
			if (iparams==NULL || curValues==NULL) rc=RC_NOMEM;
				...
				if (rc==RC_OK && (is=new(qctx.ses,nRanges,*cidx) IndexScan(qctx.qx,*cidx,flags,nRanges,qctx.flg))==NULL) rc=RC_NOMEM;
				...
				if (rc==RC_OK) {
					...
					if (cqv->cond!=NULL && cqry->hasParams()) {
						...
						if ((cqv->cond->getFlags()&EXPR_PARAMS)!=0) {
							if (qctx.ncqs>=sizeof(qctx.condQs)/sizeof(qctx.condQs[0]) || (cqry=new(qctx.ses) Stmt(0,qctx.ses))==NULL) {rc=RC_NOMEM; break;}
							if (((Stmt*)cqry)->addVariable()==0xFF) {((Stmt*)cqry)->destroy(); rc=RC_NOMEM; break;}
							QueryWithParams &qs=qctx.condQs[qctx.ncqs++]; qs.qry=(Stmt*)cqry; qs.params=(Value*)cs.params; qs.nParams=cs.nParams;
							if ((cqry->top->cond=Expr::clone(cqv->cond,qctx.ses))==NULL) {((Stmt*)cqry)->destroy(); rc=RC_NOMEM; break;}
						}
					}
	...
	if (rc==RC_OK) {
		...
		if (qctx.nqs>nqs0) {
			if (!expr.isEmpty() && expr.type!=VT_COLLECTION) {
				if (qctx.ncqs>=sizeof(qctx.condQs)/sizeof(qctx.condQs[0]) ||
											(qctx.src[qctx.nqs]=new(qctx.ses) ExprScan(qctx.qx,expr,qctx.flg))==NULL) rc=RC_NOMEM; else qctx.nqs++;
			}
			...
			if (rc==RC_OK && primary!=NULL) {if ((qq=new(qctx.ses) HashOp(primary,qctx.src[nqs0]))!=NULL) {qctx.src[nqs0]=qq; primary=NULL;} else rc=RC_NOMEM;}
		...
		if (rc==RC_OK) {
			if (fArrayFilter) {if ((qq=new(qctx.ses) ArrayFilter(qctx.src[nqs0],expr.varray,expr.length))!=NULL) qctx.src[nqs0]=qq; else rc=RC_NOMEM;}
			if (rc==RC_OK && path!=NULL && nPathSeg!=0) {
				...
				if (rc==RC_OK) {
					if (qctx.src[nqs0]->props==NULL || ps[0].nPids!=0 && BIN<PropertyID>::find(ps[0].nPids==1?ps[0].pid:ps[0].pids[0],qctx.src[nqs0]->props[0].props,qctx.src[nqs0]->props[0].nProps)==NULL) {
						...
						QueryOp *q=new(qctx.ses,1) LoadOp(qctx.src[nqs0],&pl,1,qctx.flg); if (q!=NULL) qctx.src[nqs0]=q; else rc=RC_NOMEM;
					}
					if ((qq=new(qctx.ses) PathOp(qctx.src[nqs0],ps,nPathSeg,qctx.flg))!=NULL) qctx.src[nqs0]=qq; else rc=RC_NOMEM;
				}
			}
		}
	}
...
RC QBuildCtx::mergeN(QueryOp *&res,QueryOp **o,unsigned no,QUERY_SETOP op)
{
	...
	return (res=new(ses,no) MergeIDs(qx,o,no,op,flg))!=NULL?RC_OK:RC_NOMEM;
}

RC QBuildCtx::merge2(QueryOp *&res,QueryOp **qs,const CondEJ *cej,QUERY_SETOP qo,const Expr *cond)
{
	...
	try {if ((res=new(ses,nej) MergeOp(qs[0],qs[1],cej,nej,qo,cond,flg|ff))==NULL) return RC_NOMEM;} catch (RC rc) {return rc;}
	return RC_OK;
}

RC QBuildCtx::nested(QueryOp *&res,QueryOp **qs,const Expr *cond)
{
	...
	return (res=new(ses) NestedLoop(qs[0],qs[1],flg))!=NULL?RC_OK:RC_NOMEM;
}
...
RC QBuildCtx::filter(QueryOp *&qop,const Expr *cond,const PropertyID *props,unsigned nProps,const CondIdx *condIdx,unsigned ncq)
{
	...
	try {
		Filter *flt=new(ses) Filter(qop,nqs,flg); if (flt==NULL) return RC_NOMEM;
		bool fOK=true; 
		if (cond!=NULL) {
			if ((flg&QO_VCOPIED)==0) flt->cond=cond; else if ((flt->cond=Expr::clone(cond,ses))==NULL) fOK=false;
		}
		if (props!=NULL && nProps!=0) {
			if ((flg&QO_VCOPIED)==0) {flt->rprops=(PropertyID*)props; flt->nrProps=nProps;}
			else if ((flt->rprops=new(ses) PropertyID[flt->nrProps=nProps])==NULL) fOK=false;
			else memcpy(flt->rprops,props,nProps*sizeof(PropertyID));
		}
		if ((flg&QO_CLASS)==0) {
		...
		if (!fOK) {delete flt; return RC_NOMEM;} else qop=flt;
...
RC QBuildCtx::load(QueryOp *&qop,const PropListP& plp)
{
	...
	// all pins, all props
	QueryOp *q=new(ses,plp.nPls) LoadOp(qop,plp.pls,plp.nPls,flg); if (q==NULL) return RC_NOMEM;
...
{
	RC rc=RC_OK; Values *outs=qv->outs,*vv; unsigned nOuts=qv->nOuts;
	if (outs!=NULL && nOuts!=0 && (flg&QO_VCOPIED)!=0) {
		if ((vv=new(ses) Values[nOuts])==NULL) return RC_NOMEM;
	...
	try {return (qop=new(ses) TransOp(qop,outs,nOuts,qv->aggrs,qv->groupBy,qv->nGroupBy,qv->having,f))!=NULL?RC_OK:RC_NOMEM;}
	catch (RC rc) {return rc;}
}
...
RC PropListP::checkVar(uint16_t var)
{
	if (var>=nPls) {
		if (var>=sizeof(pbuf)/sizeof(pbuf[0])) {
			if (pls!=pbuf) {if ((pls=(PropList*)ma->realloc(pls,(var+1)*sizeof(PropList)))==NULL) return RC_NOMEM;}
...
RC PropListP::merge(uint16_t var,const PropertyID *pids,unsigned nPids,bool fForce,bool fFlags)
{
	...
	if (pl.props==NULL) {
		...
		if (pl.fFree) {if ((pl.props=(PropertyID*)ma->realloc(pl.props,(pl.nProps+ex)*sizeof(PropertyID)))==NULL) return RC_NOMEM;}
