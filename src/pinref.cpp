...
PINRef::PINRef(ushort si,const byte *p,size_t l) : stID(si),count(1),def(0)
{
	...
	afy_dec32(p,ad.pageID); if (p>=end) throw RC_CORRUPTED;
	afy_dec16(p,ad.idx);
	if (p<end) {
		...
		if ((dscr&0x01)!=0) {afy_dec16(p,si); if (p>end) throw RC_CORRUPTED;}
		if ((dscr&0x02)!=0) {afy_dec32(p,id.ident); if (p>end) throw RC_CORRUPTED;}
		...
		if ((dscr&0x40)!=0) {
			...
			if ((dscr2&0x01)!=0) {if ((dscr2&0x02)==0) si2=stID; else {afy_dec16(p,si2); if (p>end) throw RC_CORRUPTED;}}
			...
			else {afy_dec32(p,id2.ident); if (p>end) throw RC_CORRUPTED;}
...
byte PINRef::enc(byte *p) const
{
	...
	if ((dscr|def)!=0) {
		if ((dscr&0x01)!=0) afy_enc16(p,si);
		if ((dscr&0x02)!=0) afy_enc32(p,id.ident);
		if (def!=0) {
			...
			if ((def&(PR_PID2|PR_U2))!=0) {
				...
				afy_enc32(p,ad.pageID); afy_enc16(p,ad.idx);
				if (si2!=si) {dscr2|=0x01; if (si2!=stID) {dscr2|=0x02; afy_enc16(p,si2);}}
				if (id2.ident!=id.ident) {dscr2|=0x04; if (id2.ident!=STORE_OWNER) {dscr2|=0x08; afy_enc32(p,id2.ident);}}
			...
			if ((dscr&0x10)!=0) afy_enc32r(p,count);
...
RC PINRef::getPID(const byte *p,size_t l,ushort si,PID& id,PageAddr *paddr)
{
	...
	if ((dscr&0x80)!=0) {
		...
		if ((dscr&0x01)!=0) afy_dec16(p,si);
		if ((dscr&0x02)!=0) {if (p>end) return RC_CORRUPTED; afy_dec32(p,id.ident);}
...
RC PINRef::adjustCount(byte *p,size_t& l,uint32_t cnt,byte *buf,bool fDec)
{
	...
	if ((dscr&0x80)==0) {
		...
		else {memcpy(buf,p0,l); p=buf+l; ++cnt; afy_enc32r(p,cnt); *p=0xB0; l=ulong(p-buf+1); rc=RC_TRUE;}
	} else {
...
int PINRef::cmpPIDs(const byte *p1,unsigned l1,const byte *p2,unsigned l2)
{
	...
	uint32_t u1,u2; afy_dec32(p1,u1); afy_dec32(p2,u2);
	...
	if ((e1[-1]&0x83)>0x80 || (e2[-1]&0x83)>0x80) {
		if (((e1[-1]|e2[-1])&0x01)!=0) {
			if ((e1[-1]&0x81)!=0x81) x1=StoreCtx::get()->storeID; else afy_dec16(p1,x1);
			if ((e2[-1]&0x81)!=0x81) x2=StoreCtx::get()->storeID; else afy_dec16(p2,x2);
			if ((c=cmp3(x1,x2))!=0) return c; assert(p1<e1 && p2<e2);
		}
		if (((e1[-1]|e2[-1])&0x02)!=0) {
			if ((e1[-1]&0x82)!=0x82) u1=STORE_OWNER; else afy_dec32(p1,u1);
			if ((e2[-1]&0x82)!=0x82) u2=STORE_OWNER; else afy_dec32(p2,u2);
			return cmp3(u1,u2);
		}
	}
	return 0;
