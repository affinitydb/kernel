...
RC FSMgr::init(FileID fid)
{
	...
	lExtentTable = ulong(ctx->theCB->nDirPages*lPage/sizeof(ExtentDirPage::DirSlot));
	...
	for (ulong i=0; i<ctx->theCB->nDirPages; i++) {
		...
		if (pb==NULL) report(MSG_ERROR,"Cannot read free page dir page %d(%08X)\n",i,ctx->theCB->dirPages[i]);
			...
			for (ulong i=0; i<dp->nslots; i++) {
				ExtentDirPage::DirSlot *ds = &((ExtentDirPage::DirSlot*)(dp+1))[i];
				if (nExtents>=lExtentTable) {
					lExtentTable += lExtentTable/4;
					extentTable = (ExtentInfo**)realloc(extentTable,lExtentTable*sizeof(ExtentInfo*),STORE_HEAP);
				...
				if ((ds->nPages&FREEPAGEFLAG)!=0) {
					...
					ext->nFreePages = ~0ul;
					ext->maxContiguous = ~0ul;
				}
			}
			slotsLeft = ExtentDirPage::maxDirSlots(lPage) - dp->nslots;
		}
	}
...
void FSMgr::close()
{
	...
	lock.lock(RW_X_LOCK);
	for (ulong i=0; i<nExtents; i++) if ((ext=extentTable[i])!=NULL && (ext->state&EXTMAP_MODIFIED)!=0) {
		if (ext->dirPage!=dirPage) {
			dirPage = ext->dirPage;
			dir = ctx->bufMgr->getPage(dirPage,&extentDirPage,PGCTL_XLOCK,dir);
			ds = dir==NULL ? NULL : (ExtentDirPage::DirSlot*)(((ExtentDirPage::ExtentDirHeader*)dir->getPageBuf())+1);
		}
		if (ds!=NULL) {
			if (ext->pos>=((ExtentDirPage::ExtentDirHeader*)dir->getPageBuf())->nslots) {
...
FSMgr::ExtentInfo *FSMgr::findExtent(PageID pid,bool fReserve)
{
	...
	for (ulong n=nExtents; n>0; ) {
		ulong k=n>>1; ExtentInfo *qq=tab[k];
...
RC FSMgr::allocPages(ulong nPages,PageID *buf,PBlock **pAllocPage)
{
	Session *ses=Session::getSession(); ulong nAlloc=0,ndw=0;
	if (!ses->inWriteTx() && pAllocPage==NULL) return RC_READTX;
	if (ses!=NULL && (ulong)ses->tx.defFree!=0) while ((buf[nAlloc]=ses->tx.defFree.pop())!=INVALID_PAGEID)
	...
	for (PBlock *pb=NULL;;) {
		...
		uint32_t *pBmp=extentMapPage.getBMP((ExtentMapPage::ExtentMapHeader*)pb->getPageBuf());
		for (ulong limit=(ext->nPages+BITSPERELT-1)/BITSPERELT; ext->firstFree<limit; ++ext->firstFree,sht+=BITSPERELT) {
			uint32_t bmp=pBmp[ext->firstFree],mask=ext->firstFree+1==limit?(1<<ext->nPages%BITSPERELT)-1:~0u;
			while ((bmp&mask)!=mask) {
				ulong bitN=pop(bmp&~(bmp+1)); pBmp[ext->firstFree]=bmp|=1<<bitN;
...
{
	ulong nNewPages=ctx->theCB->nPagesPerExtent;
	...
	if (ctx->theCB->maxSize!=0) {
		...
		if (fullSize>=ctx->theCB->maxSize || fullSize+uint64_t(nNewPages)*lPage>ctx->theCB->maxSize &&
											(nNewPages=ulong((ctx->theCB->maxSize-fullSize)/lPage))==0) 
	...
	ExtentDirPage::ExtentDirHeader *edh=(ExtentDirPage::ExtentDirHeader*)dir->getPageBuf();
	ExtentDirPage::DirSlot *ds=(ExtentDirPage::DirSlot*)(edh+1);
	ds[ext->pos].extentStart=ext->extentStart;
	ds[ext->pos].nPages=ext->nPages|FREEPAGEFLAG;
	if ((ext->state&EXTMAP_MODIFIED)==0) edh->nslots++;
...
RC ExtentMapPage::update(PBlock *pb,size_t len,ulong info,const byte *rec,size_t lrec,ulong flags,PBlock *)
{
	...
	FSMgr::ExtentInfo *ext=(flags&(TXMGR_UNDO|TXMGR_RECV))!=0?ctx->fsMgr->findExtent(emp->hdr.pageID+1):NULL;
	assert(ext==NULL||ext->extentStart==emp->hdr.pageID);
	if (rec==NULL || lrec==0) {
		ulong bitN=info&~RESETBIT;
		...
		for (ulong i=0; i*sizeof(uint32_t)<lrec; i+=2) {
...
void ExtentDirPage::initPage(byte *frame,size_t len,PageID pid)
{
	ExtentDirHeader *pH = (ExtentDirHeader*)frame;

	pH->pageID		= pid;
	pH->len			= (ushort)len;
	pH->magic		= EXTENTDIRMAGIC;
	pH->nslots		= 0;
}

bool ExtentDirPage::afterIO(PBlock *pb,size_t lPage,bool fLoad)
{
	ExtentDirHeader *pH = (ExtentDirHeader*)pb->getPageBuf();
	ExtentDirFooter *pF = (ExtentDirFooter*)(pb->getPageBuf()+lPage-sizeof(ExtentDirFooter));

	const char *what;
	if (pH->crc!=CRC32::_CRC.checksum(pb->getPageBuf()+sizeof(CRC),lPage-sizeof(CRC)-sizeof(ExtentDirFooter)))
		what="incorrect CRC";
	else if (pH->pageID!=pb->getPageID()) what="incorrect PageID";
	else if (pH->len!=ushort(lPage)) what="incorrect length";
	else if (pH->magic!=EXTENTDIRMAGIC) what="incorrect 'magic'";
	else if (pH->nslots>maxDirSlots(lPage)) what="incorect number of slots";
	else if (pH->crc!=pF->crc) what="incorrect trailer";
	else return true;
	report(MSG_ERROR,"Page %08X corrupt after read: %s\n",pb->getPageID(),what);
	return false;
}
...
{
	ExtentDirHeader *pH = (ExtentDirHeader*)frame;
	ExtentDirFooter *pF = (ExtentDirFooter*)(frame+len-sizeof(ExtentDirFooter));

	const char *what;
	if (pH->pageID!=pid) what="incorrect PageID";
	else if (pH->len!=ushort(len)) what="incorrect length";
	else if (pH->magic!=EXTENTDIRMAGIC) what="incorrect 'magic'";
	else if (pH->nslots>maxDirSlots(len)) what="incorect number of slots";
	else {
		pH->crc	= CRC32::_CRC.checksum(frame+sizeof(CRC),len-sizeof(CRC)-sizeof(ExtentDirFooter));
		pF->crc	= pH->crc;
		return true;
	}
...
RC FreeSpacePage::update(PBlock*,size_t len,ulong info,const byte *rec,size_t lrec,ulong flags,PBlock*)
{
	return RC_OK;
}
