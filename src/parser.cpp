...
const OpDscr SInCtx::opDscr[] = 
{
	...
	{0,		LX_RCBR,	S_L(","),			{40,  2},	{2,	2}	},		 //OP_ARRAY
...
{
	{S_L("")}, {S_L("STRING")}, {S_L("BSTR")}, {S_L("URL")}, {S_L("ENUM")},
	{S_L("INT")}, {S_L("UINT")}, {S_L("INT64")}, {S_L("UINT64")}, {S_L("DECIMAL")}, {S_L("FLOAT")}, {S_L("DOUBLE")},
	...
	{S_L("EXPR")}, {S_L("QUERY")}, {S_L("ARRAY")}, {S_L("CNAVIGATOR")}, {S_L("RANGE")},
	{S_L("STREAM")}, {S_L("CURRENT")}, {S_L("PARAM")}, {S_L("VARREF")}, {S_L("EXPRTREE")}
};
...
static RC renderOrder(const OrderSegQ *seg,unsigned nSegs,SOutCtx& out)
{
	RC rc;
...
//					if (size_t(~v.ui)>=sizeof(elementIDs)/sizeof(elementIDs[0])) {
//						size_t l=sprintf(out.cbuf,"%u",v.ui); if (!out.append(out.cbuf,l)) return RC_NORESOURCES;
//					} else if (!out.append(elementIDs[~v.ui].s,elementIDs[~v.ui].l)) return RC_NORESOURCES;
					break;
				case OP_RENAME:
					if (!out.append("=",1)) return RC_NORESOURCES;
	...
	case STMT_DELETE: case STMT_UNDELETE:
		if (fFrom) {
			if (top->type!=QRY_SIMPLE || ((SimpleVar*)top)->classes!=NULL || ((SimpleVar*)top)->pids!=NULL) {
				if (!out.append("\n"FROM_S" ",sizeof(FROM_S)+1)) return RC_NORESOURCES;
				if ((rc=top->render(RP_FROM,out))!=RC_OK) break;
			} else {
				if (((SimpleVar*)top)->condIdx!=NULL || ((SimpleVar*)top)->props!=NULL) out.flags|=SOM_WHERE;
...
RC QVar::render(SOutCtx& out) const
{
	...
	if (dtype==DT_ALL) {
		...
		if (dtype==DT_DISTINCT_VALUES && !out.append(VALUES_S" ",sizeof(VALUES_S))) return RC_NORESOURCES;
	}
	if (dscr!=NULL && nDscr>0) {
		//???
	...
	if (type!=QRY_SIMPLE || ((SimpleVar*)this)->classes!=NULL || ((SimpleVar*)this)->pids!=NULL || ((SimpleVar*)this)->path!=NULL) {
		...
		if (((SimpleVar*)this)->condIdx!=NULL || ((SimpleVar*)this)->props!=NULL) out.flags|=SOM_WHERE;
	...
	if (nHavingConds>0) {
		...
		Expr *const *pcnd = nHavingConds==1?&havingCond:havingConds;
		const int prty=(out.flags&SOM_AND)!=0||nHavingConds>1?SInCtx::opDscr[OP_LAND].prty[0]:0;
		for (unsigned i=0; i<nHavingConds; out.flags|=SOM_AND,i++) {
...
RC QVar::render(RenderPart part,SOutCtx& out) const
{
	...
	case RP_WHERE:
		if (nConds>0) {
			Expr *const *pcnd=nConds==1?&cond:conds; out.cvar=this;
...
{
	RC rc; assert(type==QRY_JOIN||type==QRY_LEFTJOIN||type==QRY_RIGHTJOIN||type==QRY_OUTERJOIN);
	...
	for (unsigned i=0; i<nVars; ++i) {
		...
		if (i!=0 && part==RP_FROM) {
			...
			case QRY_JOIN: break;
			case QRY_LEFTJOIN: if (!out.append(" "LEFT_S" ",sizeof(LEFT_S)+1)) return RC_NORESOURCES; break;
			case QRY_RIGHTJOIN: if (!out.append(" "RIGHT_S" ",sizeof(RIGHT_S)+1)) return RC_NORESOURCES; break;
			case QRY_OUTERJOIN: if (!out.append(" "OUTER_S" ",sizeof(OUTER_S)+1)) return RC_NORESOURCES; break;
			}
...
RC SimpleVar::render(RenderPart part,SOutCtx& out) const
{
	...
	default: break;
	case RP_FROM:
		if (condIdx!=NULL || props!=NULL) out.flags|=SOM_WHERE;
	...
	case RP_WHERE:
		if (props!=NULL && nProps>0) {
			...
			for	(unsigned i=0; i<nProps; i++) {
				...
				if ((rc=out.renderName(props[i],NULL,this))!=RC_OK) return rc;
				if (!out.append(")",1)) return RC_NORESOURCES;
			}
			out.flags|=SOM_AND;
		}
		...
		for (CondIdx *ci=condIdx; ci!=NULL; ci=ci->next,out.flags|=SOM_AND) {
			...
			if (ci->expr==NULL) {
				...
				if ((ci->expr->expr->render(SInCtx::opDscr[OP_EQ].prty[0],out))!=RC_OK) return rc;
			}
...
RC ExprTree::render(int prty,SOutCtx& out) const
{
	...
	const OpDscr *od=&SInCtx::opDscr[o]; bool f,fRange; ushort i; PathSeg pseg;
	switch (o) {
	case OP_EXISTS:
	...
	case OP_IN:
		...
		break;
	case OP_CAST:
		if (operands[1].type!=VT_INT && operands[1].type!=VT_UINT || operands[1].i<=0 || operands[1].i>=VT_ALL) return RC_CORRUPTED;
		if (!out.append("CAST(",5)) return RC_NORESOURCES;
		if ((rc=render(operands[0],prty,out))!=RC_OK) return rc;
		if (!out.append(" "AS_S" ",sizeof(AS_S)+1) || 
			!out.append(typeName[operands[1].i].s,typeName[operands[1].i].l) ||
			!out.append(")",1)) return RC_NORESOURCES;
		break;
	case OP_TRIM:
	...
	default:
		...
		if ((od->flags&_F)!=0) {
			...
			else if ((flags&NOT_BOOLEAN_OP)!=0 && (!out.append(NOT_S,sizeof(NOT_S)-1) || !out.append(" ",1))) return RC_NORESOURCES;
			do {
				if (!out.append(od->str,od->lstr) || !out.append(" ",1)) return RC_NORESOURCES;
				if ((rc=render(operands[idx],od->prty[0],out))!=RC_OK) return rc;
			} while (++idx<nops);
...
RC ExprTree::toPathSeg(PathSeg& ps,MemAlloc *ma) const
{
	...
	ps.eid=STORE_COLLECTION_ID; ps.filter=NULL; ps.cls=STORE_INVALID_CLASSID; ps.fLast=(flags&FILTER_LAST_OP)!=0;
...
{
	ExprTree *et=NULL; RC rc=decompile(et,out.getSession());
...
RC SOutCtx::renderValue(const Value& v,bool fJ) {
	...
	if (fJ && v.type>VT_DOUBLE && v.type!=VT_ARRAY && v.type!=VT_COLLECTION && v.type!=VT_REFID) {
	...
	case VT_EXPRTREE:
		...
		break;
	case VT_EXPR:
		if (!append("$(",2) || (rc=((Expr*)v.expr)->render(0,*this))==RC_OK && !append(")",1)) rc=RC_NORESOURCES;
		break;
	case VT_STMT:
		if (!append("${",2) || (rc=((Stmt*)v.stmt)->render(*this))==RC_OK && !append("}",1)) rc=RC_NORESOURCES;
		break;
	case VT_PARAM:
		l=sprintf(cbuf,PARAM_PREFIX"%d",v.refPath.refN); if (!append(cbuf,l)) return RC_NORESOURCES;
		if (v.refPath.type<VT_ALL && v.refPath.type!=VT_ANY) {
			if (!append("(",1)) return RC_NORESOURCES;
			if (!append(typeName[v.refPath.type].s,typeName[v.refPath.type].l)) return RC_NORESOURCES;
			if ((v.refPath.flags&ORD_DESC)!=0 && !append(","DESC_S,sizeof(DESC_S))) return RC_NORESOURCES;
			if ((v.refPath.flags&(ORD_NULLS_BEFORE|ORD_NULLS_AFTER))!=0) {
				if (!append(","NULLS_S" ",sizeof(NULLS_S)+1)) return RC_NORESOURCES;
				if ((v.refPath.flags&ORD_NULLS_BEFORE)!=0) {if (!append(FIRST_S,sizeof(FIRST_S)-1)) return RC_NORESOURCES;}
				else if (!append(LAST_S,sizeof(LAST_S)-1)) return RC_NORESOURCES;
			}
			if (!append(")",1)) return RC_NORESOURCES;
		}
		if (v.length==0 || (rc=renderName(v.refPath.id,URIID_PREFIX))==RC_OK) rc=renderElement(v.eid,*this);
		break;
	case VT_VARREF:
		qv=NULL;
		if (v.length==0 || (flags&SOM_VAR_NAMES)!=0) {
			if ((qv=cvar!=NULL?cvar->getRefVar(v.refPath.refN):(QVar*)0)==NULL) return RC_CORRUPTED;
			if (v.length==0) {rc=renderVarName(qv); break;}
		}
		if (v.length==0 || (rc=renderName(v.refPath.id,NULL,qv))==RC_OK) rc=renderElement(v.eid,*this);
		break;
	case VT_ENUM:
	case VT_DECIMAL:
		//???
...
const char *SInCtx::errorMsgs[SY_ALL] = {
	...
	"missing '='", "missing class name", "unknown qname prefix",
};
...
TLx SInCtx::lex()
{
	...
	case LX_QUOTE:
		for (cbeg=beg=ptr,ls=0,buf=NULL,ch=ptr[-1]; ptr<end; ) if (*ptr++==ch) {
			if (ptr>=end || *ptr!=ch) {
				if (buf!=NULL && cbeg+1<ptr) {memcpy(buf+ls,cbeg,ptr-cbeg-1); buf[ls+(ptr-cbeg)-1]='\0';}
				v.set(beg,uint32_t(ptr-cbeg-1+ls)); if (buf!=NULL) v.flags=mtype;
			...
			} else if (cbeg+1<ptr) {memcpy(buf+ls,cbeg,ptr-cbeg-1); ls+=ptr-cbeg-1;}
			cbeg=++ptr;
		}
		errpos=ptr; throw lx==LX_DQUOTE?SY_MISDQU:SY_MISQUO;
	case LX_SELF:
		if (ptr>=end || !isxdigit(*(byte*)ptr)) return LX_SELF;
	...
	case LX_DOLLAR:
		if (ptr<end) {
			...
			case LX_DIGIT:
				if (ids==NULL || nids==0) throw SY_INVURN;
				for (u=*ptr-'0'; ++ptr<end && isD(ch=(byte)*ptr);)
					if (TN_CHECK(u,ch,nids-1)) u=u*10+ch; else throw SY_INVURN;
...
TLx SInCtx::parsePrologue()
{
	...
	if (lx==LX_KEYW) {
		...
		for (QName qn; lx==LX_KEYW && v.op==KW_PREFIX; lx=lex()) {
			...
			if ((rc=mv_binscmp<QName,unsigned>(qNames,nQNames,qn,ma))!=RC_OK) throw rc;
		}
	}
	return lx;
}
	...
	if (ses!=NULL && v.type==VT_STRING) {
		...
		if (prefix!=NULL && lPrefix!=0) {
			...
			if (lastQN>=nQNames || (pq=&qNames[lastQN])->lq!=lPrefix || memcmp(prefix,pq->qpref,lPrefix)!=0)
				pq=mv_bsrcmp<QName,const QName&>(qn,qNames,nQNames);
			if (pq==NULL && (pq=mv_bsrcmp<QName,const QName&>(qn,ses->qNames,ses->nQNames))==NULL) throw SY_UNKQPR;
...
void SInCtx::parse(Value& res,const QVarRef *vars,unsigned nVars,unsigned pflags)
{
	...
	for (TLx lx=(pflags&PRS_TOP)==0?lex():parsePrologue(),lx2;;lx=lex()) {
		...
		case LX_IDENT:
			...
			else if (nVars<=1) {mapURI(); assert(v.type==VT_URIID); URIID uid=v.uid; v.setVarRef(nVars>0?vars->var->getID():0,uid);}
		...
		case LX_PERIOD:
			...
			nextLex=lx; lx=OP_PATH; est=0; break;
		case LX_EXCL:
		...
		case LX_COLON:
			...
			if ((lx=lex())!=LX_CON || v.type!=VT_INT) {
				if (ops.top().lx!=LX_FILTER) throw SY_MISNUM;
				switch (lx) {
				...
				if ((lx=lex())!=LX_LPR) nextLex=lx;
					...
					for (unsigned i=1; ; i++)
						...
						else if (typeName[i].l==v.length && cmpncase(v.str,typeName[i].s,v.length)) {vv.refPath.type=(byte)i; break;}
					if ((lx=lex())==LX_COMMA) {
						if ((lx=lex())==LX_RPR) throw SY_SYNTAX;
						if (lx==LX_KEYW && (v.op==KW_DESC||v.op==KW_ASC)) {
							if (v.op==KW_DESC) vv.refPath.flags|=ORD_DESC; 
						...
						if (lx!=LX_RPR) {
							...
							if (v.length==sizeof(FIRST_S)-1 && cmpncase(v.str,FIRST_S,sizeof(FIRST_S)-1)) vv.refPath.flags|=ORD_NULLS_BEFORE;
							else if (v.length==sizeof(LAST_S)-1 && cmpncase(v.str,LAST_S,sizeof(LAST_S)-1)) vv.refPath.flags|=ORD_NULLS_AFTER;
		...
		case LX_QUEST:
			...
			ops.pop(); if ((po=ops.top(1))->lx==LX_PATHQ) po=ops.top(2); assert(po->lx==OP_PATH); 
			po->flag|=QUEST_PATH_OP; est=1; continue;
		case LX_IS:
		...
		case OP_PLUS:
			...
			if (ops.top().lx==LX_REPEAT && (nextLex=lex())==LX_RCBR) {
				ops.pop(); if ((po=ops.top(1))->lx==LX_PATHQ) po=ops.top(2); assert(po->lx==OP_PATH); 
				po->flag|=PLUS_PATH_OP; nextLex=LX_ERR; est=1;
			}
			continue;
		case OP_MINUS:
		...
		case OP_MUL:
			...
			case LX_RPR:
				if (ops.top().lx!=OP_COUNT || (mode&SIM_SELECT)==0) throw SY_MISCON;
			...
			case LX_RCBR:
				...
				ops.pop(); if ((po=ops.top(1))->lx==LX_PATHQ) po=ops.top(2);
		...
		case LX_LPR:
			if (est!=0) {lx=OP_CALL; est=0;}
				...
				if (v.op==KW_INSERT) {
					...
					for (;;) {
						stmt=parse(); assert(stmt!=NULL && stmt->getOp()==STMT_INSERT); stmts+=stmt;
						if ((lx=lex())!=LX_SEMI) break;
						if ((nextLex=lex())!=LX_KEYW || v.op!=KW_INSERT) {sy=SY_SYNTAX; break;}
					}
					if (sy!=SY_ALL || (sy=sy=SY_MISRPR,lx)!=LX_RPR) {for (unsigned i=stmts; i--!=0;) stmts[i]->destroy(); throw sy;}
					...
					stmt=parse(); assert(stmt!=NULL && stmt->getOp()==STMT_QUERY);
		...
		for (const byte prty=od->prty[0];;) {
			...
			case LX_COMMA:
				...
				if (op.lx==LX_LPR || op.lx==LX_LCBR) {
			case LX_LCBR:
					if (op.lx==LX_LPR) {if (lx!=LX_RPR) throw SY_MISRPR;} else if (lx!=LX_RCBR) throw SY_MISRCBR;
					vals=oprs.pop(nvals); assert(vals!=NULL);
					if ((rc=ExprTree::normalizeArray(vals,nvals,vv,ma,ses!=NULL?ses->getStore():StoreCtx::get()))!=RC_OK) throw rc;
			...
			case LX_BETWEEN:
				...
				ops.push(OpF(LX_BETWEEN,0)); ops.push(OpF(LX_BAND,0)); goto next_lx;
			case LX_BAND:
				...
				oprs.push(vv); od2=&opDscr[op.lx=OP_IN]; break;
			case LX_FILTER:
				if (lx!=LX_RBR) throw SY_MISRBR; vals=oprs.top(2); assert(vals!=NULL);
				if (vals[0].type!=VT_REFIDPROP || vals[1].type!=VT_INT && vals[1].type!=VT_UINT) ops.push(OpF(LX_PATHQ,0));
				else {((RefVID*)vals[0].refId)->eid=vals[1].ui; vals[0].type=VT_REFIDELT; oprs.pop();}
				goto next_lx;
			case LX_REPEAT:
			lx_repeat:
				...
				if (nvals==3) {
					...
					//...
				} else throw SY_MANYARG;
				ops.push(OpF(LX_PATHQ,1)); goto next_lx;
			case LX_PATHQ:
				nvals=3;
				if ((op=ops.pop()).lx==LX_PATHQ) {
					if (op.flag==0) {vals=oprs.top(2); vv=vals[0]; vals[0]=vals[1]; vals[1]=vv; op.flag|=FILTER_LAST_OP;}
					nvals++; op=ops.pop();
				} else if (op.flag!=0) {
					// add filter with eid==STORE_COLLECTION_ID or flags|=NO_FILTER_OP?
					...
					case VT_VARREF: 
						if (vals[0].length!=0||(mode&SIM_SELECT)!=0) break;
						vals[0].refPath.id=vals[1].uid; vals[0].length=1; vals[0].eid=vals[2].ui; oprs.pop(2); continue;
					case VT_REFID:
			...
			case OP_PATH:
				if ((pflags&PRS_PATH)==0) {
					...
					case VT_VARREF:
						if (vals[0].length!=0||(mode&SIM_SELECT)!=0) break;
						vals[0].refPath.id=vals[1].uid; vals[0].length=1; oprs.pop(); continue;
					case VT_REFID:
						if ((rv=new(ma) RefVID)==NULL) throw RC_NORESOURCES;
		...
		case OP_IS_A:
			lx2=lex(); assert(lx2==LX_IDENT&&v.type==VT_URIID||lx2==LX_CON&&v.type==VT_PARAM);
		...
		case OP_EXTRACT:
			...
			ops.push(OpF(lx,opf)); break;
		case OP_IN: 
			ops.push(OpF(lx,opf)); opf=0; if ((lx=lex())!=LX_LPR) {nextLex=lx; break;}
		case OP_EXISTS:
		...
		case LX_FILTER: case LX_REPEAT:
			if ((lx2=ops.top().lx)!=OP_PATH) {
				if (lx2==LX_PATHQ) {
					// check OP_PATH before and flag
				} else if (oprs.top().type==VT_VARREF) {
					// check unbound
				} else throw SY_SYNTAX;
			}
			ops.push(OpF(lx,opf)); break;
...
QVarID SInCtx::parseQuery(Stmt*& res,bool fNested)
{
	...
	for (TLx lx=fNested?lex():parsePrologue();;lx=lex()) {
		...
		if (type!=QRY_INTERSECT) for (OpD op;;) {
			if (ops.isEmpty()) {
				...
				if (!fNested) {
					if ((lx=lex())==LX_KEYW && v.op==KW_ORDER) lx=parseOrderBy(res,INVALID_QVAR_ID);
...
QVarID SInCtx::parseSelect(Stmt* stmt,bool fMod)
{
	...
	QVarID var=INVALID_QVAR_ID; DistinctType dt=DT_DEFAULT; DynArray<Value> trs(ma);
	if (!fMod) {
		...
		if (lx==OP_MUL) {
			...
			nextLex=lx; mode|=SIM_SELECT;
			// multiple pins???
			do {
				Value v; parse(v); v.property=STORE_INVALID_PROPID;
				if ((lx=lex())==LX_KEYW && v.op==KW_AS) {
					if (lex()!=LX_CON || v.op!=VT_STRING && v.op!=VT_URIID) throw SY_MISPROP;
					mapURI(); v.property=v.uid;
				} else if (lx==LX_CON && (v.op==VT_STRING || v.op==VT_URIID)) {
					mapURI(); v.property=v.uid;
				} else nextLex=lx;
				trs+=v;
	...
	if (!fMod && lx==LX_KEYW && v.op==KW_GROUP && (lx=parseOrderBy(stmt,var))==LX_KEYW && v.op==KW_HAVING) {
		ExprTree *pe=parseCondition(NULL,0);
		if (pe!=NULL) {rc=stmt->addCondition(var,pe,true); pe->destroy(); if (rc!=RC_OK) throw rc;}
		lx=lex();
	}
	if (lx!=LX_EOE) nextLex=lx;
	if (trs!=0) {
		if (stmt->top!=NULL && trs==1 && trs[0].type==VT_EXPRTREE && trs[0].exprt->getOp()==OP_COUNT && trs[0].exprt->getOperand(0).type==VT_ANY) {
			trs[0].exprt->destroy(); stmt->top->stype=SEL_COUNT;
		} else {
			QVarRef qr; qr.var=stmt->top;
			for (unsigned i=0; i<trs; i++) if ((rc=resolveSelect(&qr,*(Value*)&trs[i]))!=RC_OK) throw rc;
			if ((rc=stmt->addOutput(var,trs,trs))!=RC_OK) throw rc;		// multiple descriptors???
		}
	}
	return var;
}
...
QVarID SInCtx::parseFrom(Stmt *stmt)
{
	...
	for (QVarID var=INVALID_QVAR_ID,prev=INVALID_QVAR_ID;;) {
		...
		if (jkw!=-1) {
			...
			var=stmt->join(prev,var,pe,jkw==KW_LEFT?QRY_LEFTJOIN:jkw==KW_RIGHT?QRY_RIGHTJOIN:jkw==KW_OUTER?QRY_OUTERJOIN:QRY_JOIN);
	...
	for (;;) {
		...
		if ((lx=lex())==LX_LPR) {
			...
			do {
				...
				if (w.type==VT_EXPRTREE||w.type==VT_VARREF) {sy=SY_MISCON; break;}
				if (cs.nParams>=xP) {
...
RC SInCtx::splitWhere(Stmt *stmt,QVar *qv,ExprTree *pe)
{
	...
	if (qv->type<=QRY_OUTERJOIN) {
		...
		if (pe->vrefs!=MANY_VREFS) {
			...
			for (i=0; i<jv->nVars; i++) if (jv->vars[i].var->type<=QRY_OUTERJOIN && splitWhere(stmt,jv->vars[i].var,pe)==RC_OK) return RC_OK;
		}
		//???
	}
	return RC_NOTFOUND;
...
RC SInCtx::resolveSelect(QVarRef *qv,Value& vv)
{
	if (vv.type==VT_EXPRTREE) {
		ExprTree *pe=(ExprTree*)vv.exprt;
		for (unsigned i=0; i<pe->nops; i++) {
			Value *pv=&pe->operands[i]; RC rc; Len_Str *ls;
			switch (pv->type) {
			default: break;
			case VT_EXPRTREE:
				if (((ExprTree*)pv->exprt)->vrefs!=NO_VREFS && (rc=resolveSelect(qv,*pv))!=RC_OK) return rc;
				break;
			case VT_VARREF:
				assert(pv->refPath.refN<dnames && pv->length==0);
				ls=(Len_Str*)&dnames[pv->refPath.refN];
				if (ls->s!=NULL) {
					v.set(ls->s,(uint32_t)ls->l); QVarID var=findVar(v,qv,1);
					if (var!=INVALID_QVAR_ID) {ls->s=NULL; ls->l=var|0x80000000;}
					else {mapURI(); ls->s=NULL; ls->l=v.uid;}	// check there is only one var in stmt
				}
				if ((ls->l&0x80000000)==0) {
					pv->refPath.refN=0; pv->refPath.id=(URIID)ls->l; pv->length=1;
				} else if (i==0 && pe->op==OP_PATH && pe->operands[1].type==VT_URIID && 
					(pe->nops==2 || pe->nops==3 && (pe->operands[2].type==VT_INT || pe->operands[2].type==VT_UINT))) {
					vv.setVarRef(byte(ls->l&~0x80000000),pe->operands[1].uid); if (pe->nops==3) vv.eid=pe->operands[2].ui; ma->free(pe);
				} else {
					pv->refPath.refN=byte(ls->l&~0x80000000);
				}
				break;
			}
		}
...
TLx SInCtx::parseOrderBy(Stmt *stmt,QVarID var)
{
	if (lex()!=LX_KEYW || v.op!=KW_BY) throw SY_MISBY; assert(stmt!=NULL);
	OrderSeg osegs[10],*segs=osegs; unsigned nSegs=0,xSegs=10; TLx lx; RC rc=RC_OK;
	do {
		Value w; parse(w,NULL,0,PRS_COPYV);		// vars from Transform?
		if (nSegs>xSegs) {
			if (segs!=osegs) segs=(OrderSeg*)ses->realloc(segs,(xSegs*=2)*sizeof(OrderSeg));
			else if ((segs=(OrderSeg*)ses->malloc((xSegs*=2)*sizeof(OrderSeg)))!=NULL) memcpy(segs,osegs,nSegs*sizeof(OrderSeg));
			if (segs==NULL) throw RC_NORESOURCES;
		}
		OrderSeg& sg=segs[nSegs++]; sg.expr=NULL; sg.pid=STORE_INVALID_PROPID; sg.flags=sg.lPrefix=0; long lstr;
		for (const Value *pv=&w; ;pv=&((ExprTree*)pv->exprt)->operands[0]) {
			if (pv->type==VT_URIID) {sg.pid=pv->uid; freeV(w); break;}
			if (pv->type==VT_VARREF && pv->refPath.refN==0 && pv->length==1) {sg.pid=pv->refPath.id; freeV(w); break;}
			if (pv->type==VT_EXPRTREE) {
				if (((ExprTree*)pv->exprt)->op==OP_UPPER || ((ExprTree*)pv->exprt)->op==OP_LOWER) {sg.flags|=ORD_NCASE; continue;}
				if (((ExprTree*)pv->exprt)->op==OP_SUBSTR && Expr::getI(((ExprTree*)pv->exprt)->operands[0],lstr)==RC_OK && lstr>=0) {
					if ((((ExprTree*)pv->exprt)->nops==2 || ((ExprTree*)pv->exprt)->nops==3 && lstr==0 && 
						Expr::getI(((ExprTree*)pv->exprt)->operands[1],lstr)==RC_OK) && lstr>0) {if (sg.lPrefix==0 || lstr<sg.lPrefix) sg.lPrefix=(ushort)lstr; continue;}
				}
			}
			if ((rc=ExprTree::forceExpr(w,ses,true))!=RC_OK) throw rc;
			sg.expr=(ExprTree*)w.exprt; sg.lPrefix=0; sg.flags&=~ORD_NCASE; break;
		}
	...
	if (nSegs!=0) rc=var==INVALID_QVAR_ID?stmt->setOrder(segs,nSegs):stmt->setGroup(var,segs,nSegs);
	for (unsigned i=0; i<nSegs; i++) if (segs[i].expr!=NULL) segs[i].expr->destroy();
	if (segs!=osegs) ses->free(segs); if (rc!=RC_OK) throw rc;
	return lx;
}
...
{
	if (lx==LX_CON) {
		if (v.type==VT_INT || v.type==VT_UINT) eid=v.ui; else return false;
	} else if (lx==LX_COLON) {
		switch (lx=lex()) {
		case LX_IDENT:
			if (v.type==VT_STRING) {
				if (v.length==sizeof(FIRST_S)-1 && cmpncase(v.str,FIRST_S,sizeof(FIRST_S)-1)) {eid=STORE_FIRST_ELEMENT; break;}
				if (v.length==sizeof(LAST_S)-1 && cmpncase(v.str,LAST_S,sizeof(LAST_S)-1)) {eid=STORE_LAST_ELEMENT; break;}
			}
		default:
			nextLex=lx; return false;
		case OP_MIN: eid=STORE_MIN_ELEMENT; break;
		case OP_MAX: eid=STORE_MAX_ELEMENT; break;
		}
	} else return false;
	return true;
}
...
Stmt *SInCtx::parse()
{
	Stmt *stmt=NULL; TLx lx; uint8_t op; unsigned mode; Value vals[3];
	SynErr sy=SY_ALL; RC rc=RC_OK; Value w; ElementID eid,eid2; QVarID var; Class *cls;
	switch (lx=lex()) {
	...
	case LX_KEYW:
		mode=0;
		switch (op=v.op) {
		case KW_SELECT: nextLex=lx; stmt=NULL; parseQuery(stmt,false); break;
		case KW_INSERT:
			{
				DynArray<Value,40> props(ma); mode|=SIM_DML_EXPR;
				if ((lx=lex())==LX_KEYW && (v.op==KW_FROM || v.op==KW_WHERE || v.op==KW_MATCH)) nextLex=lx;
				else if (lx==LX_LPR) {
					do {
						if (lex()!=LX_IDENT) throw SY_MISPROP;
						mapURI(); assert(v.type==VT_URIID); 
					...
					if ((lx=lex())!=LX_KEYW || v.op!=KW_NULL) {
						nextLex=lx; parse(w);
						if ((w.flags&HEAP_TYPE_MASK)!=SES_HEAP && (rc=copyV(w,w,ma))!=RC_OK) break;
				...
				if (rc==RC_OK && sy==SY_ALL) {
					...
					rc=(stmt=new(ma) Stmt(0,ma,STMT_INSERT))!=NULL?stmt->setValuesNoCopy(vals,nVals):RC_NORESOURCES;
			...
			if ((stmt=new(ma) Stmt(0,ma,STMT_UPDATE))==NULL) throw RC_NORESOURCES;
				...
				while (rc==RC_OK && sy==SY_ALL && lx==LX_KEYW) {
					...
					do {
						...
						mapURI(); assert(v.type==VT_URIID);
						URIID uid=v.uid; w.setError(); eid=STORE_COLLECTION_ID; byte eop=op;
						if ((lx=lex())==LX_LBR) {
						...
						case OP_SET:
							...
							if ((lx=lex())==LX_KEYW && v.op==KW_NULL && op==OP_SET) eop=OP_DELETE;
								...
								nextLex=lx; parse(w); if ((w.flags&HEAP_TYPE_MASK)!=SES_HEAP) rc=copyV(w,w,ma);
								if (w.type==VT_EXPRTREE) {
									ExprTree *et=(ExprTree*)w.exprt;
									if (eop==OP_SET && et->op>=OP_FIRST_EXPR && et->op<=OP_CONCAT && et->nops==2) {
										const Value& lhs=et->operands[0];
										if (lhs.type==VT_VARREF && lhs.length==1 && lhs.refPath.refN==0 && lhs.refPath.id==uid)
						...
						w.property=uid; w.op=eop; w.eid=eid;
		...
		case KW_PURGE:
			mode=MODE_PURGE;
			if ((lx=lex())!=LX_KEYW || v.op!=KW_WITH) nextLex=lx;
			else if (lex()!=LX_IDENT || v.type!=VT_STRING || v.length!=sizeof(IDS_S)-1 || !cmpncase(v.str,IDS_S,sizeof(IDS_S)-1)) throw SY_SYNTAX;
			else mode|=MODE_PURGE_IDS;
		case KW_DELETE: case KW_UNDELETE:
			if ((stmt=new(ma) Stmt(mode,ma,op==KW_UNDELETE?STMT_UNDELETE:STMT_DELETE))==NULL) throw RC_NORESOURCES;
			else {
				bool fLCBR=false;
		...
		case KW_DROP:
			...
			if ((cls=ses->getStore()->classMgr->getClass(v.uid))==NULL) rc=RC_NOTFOUND;
				...
				if ((stmt=new(ma) Stmt(MODE_PURGE,ma,STMT_DELETE))==NULL) throw RC_NORESOURCES;
		...
		case KW_START:
			...
			else if ((lx=lex())==LX_IDENT && v.length==sizeof(ONLY_S)-1 || cmpncase(v.str,ONLY_S,sizeof(ONLY_S)-1)) mode=MODE_READONLY;
			else if (lx!=LX_IDENT || v.length!=sizeof(WRITE_S)-1 || !cmpncase(v.str,WRITE_S,sizeof(WRITE_S)-1)) throw SY_SYNTAX;
			if ((stmt=new(ma) Stmt(mode,ma,STMT_START_TX))==NULL) throw RC_NORESOURCES;
			break;
		case KW_COMMIT: case KW_ROLLBACK:
			if ((lx=lex())!=LX_KEYW || v.op!=KW_ALL) nextLex=lx; else mode=MODE_ALL;
			if ((stmt=new(ma) Stmt(mode,ma,op==KW_COMMIT?STMT_COMMIT:STMT_ROLLBACK))==NULL) throw RC_NORESOURCES;
			break;
		case KW_SET:
			if ((lx=lex())==LX_KEYW && (v.op==KW_BASE || v.op==KW_PREFIX)) {
				...
				vals[1]=v;
				if ((stmt=new(ma) Stmt(mode,ma,STMT_PREFIX))==NULL) throw RC_NORESOURCES;
				...
				// read level
				if ((stmt=new(ma) Stmt(mode,ma,STMT_ISOLATION))==NULL) throw RC_NORESOURCES;
			} else throw SY_SYNTAX;
			break;
		}
		break;
...
{
	SOutCtx out(ses); assert(ses!=NULL);
	for (TLx lx=parsePrologue();;lx=lex()) {
		SynErr sy=SY_ALL; RC rc=RC_OK; Stmt *stmt; uint64_t cnt;
		...
		if ((stmt=parse())!=NULL) {
			...
			if ((rc=stmt->execute(result!=NULL?&ir:NULL,params,nParams,nProcess,nSkip,0/*mode???*/))==RC_OK && ir!=NULL) {
				while (rc==RC_OK && (ip=ir->next())!=NULL) {
					...
					if (!out.append(out.cbuf,l)) rc=RC_NORESOURCES;
						...
						unsigned nProps=ip->getNumberOfProperties();
						for (unsigned i=0; i<nProps; i++) {
							const Value *pv=ip->getValueByIndex(i);
							if (pv!=NULL && pv->property!=STORE_INVALID_PROPID) {
								if (!fPID) {if (out.append("\"",1)) fPID=true; else {rc=RC_NORESOURCES; break;}}
								else if (!out.append(", \"",3)) {rc=RC_NORESOURCES; break;}
								if ((rc=out.renderName(pv->property,NULL,NULL,true))!=RC_OK) break;
								if (!out.append("\": ",3)) {rc=RC_NORESOURCES; break;}
								if ((rc=out.renderValue(*pv,true))!=RC_OK) break;
							}
						}
...
IStmt *SessionX::createStmt(const char *qs,const URIID *ids,unsigned nids,CompilationError *ce) 
{
	try {
		...
		try {Stmt *st=in.parse(); in.checkEnd(true); return st;}
