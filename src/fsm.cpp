...
RC StartFSM::process(Session *ses)
{
	...
	if (ctx.type==VT_REF) fsm=(PIN*)ctx.pin;
		...
		if ((rc=ses->getStore()->queryMgr->getBody(cb,TVO_UPD))!=RC_OK) return rc;
	}
	return ses->getStore()->fsmMgr->process(ses,fsm,NULL,STORE_COLLECTION_ID);
}
...
RC FSMMgr::process(Session *ses,PIN *fsm,const Value *event,ElementID tid)
{
	...
	if (tid!=STORE_COLLECTION_ID) {
		...
		if (trns.type==VT_STRUCT && (pv=Value::find(PROP_SPEC_CONDITION,trns.varray,trns.length))!=NULL && 
			pv->type==VT_EXPR && !Expr::condSatisfied((Expr**)&pv->expr,1,ectx)) return RC_FALSE;
		trans=&trns;
	}
	for (const Value *pv;;) {
		// tx ??? EV_ASYNC???
