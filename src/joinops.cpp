...
RC MergeIDOp::next(PINEx &qr,const PINEx *skip)
{
	RC rc=RC_OK;
	...
	if (fOr) {
		...
		for (ulong i=0; i<nOps; i++) {
			...
			if ((qs.state&QOS_ADV)!=0) {
				qr.cleanup(); qr=PIN::defPID; last=~0u; qs.state&=~QOS_ADV;
				while ((rc=qs.qop->next(qr,skip))==RC_OK /*&& skip!=NULL && skip->epr.cmp(qr.id,skip->id,ses->getStore()->storeID)<0*/) break;
				qs.qop->release();
				if (rc!=RC_OK) {qs.state|=QOS_EOF; if (rc==RC_EOF) continue; return rc;}
				if (qr.epr.lref==0 && (rc=qr.pack())!=RC_OK) {qr.cleanup(); qr=PIN::defPID; return rc;}
				last=i; qs.epr=qr.epr; qs.epr.flags&=PINEX_LOCKED|PINEX_XLOCKED|PINEX_ACL_CHKED|PINEX_EXTPID;
			}
			if (cur==~0u || (cmp=ops[cur].epr.cmp(qs.epr))==0) {cur=i; qs.state|=QOS_ADV;}
			else if (cmp>0) {ops[cur].state&=~QOS_ADV; cur=i; qs.state|=QOS_ADV;}
		}
		if (cur==~0u) rc=RC_EOF;
		else {rc=RC_OK; if (cur!=last) {qr.cleanup(); qr=PIN::defPID; qr.epr=ops[cur].epr;}}
	} else {
		...
		for (ulong i=0,nOK=0,flags=0; nOK<nOps; ) {
			QueryOpS& qs=ops[i]; qr.cleanup(); if ((qs.state&QOS_EOF)!=0) {rc=RC_EOF; break;}
			int cmp;
			if ((rc=qs.qop->next(qr,skip))!=RC_OK) {qs.state|=QOS_EOF; qs.qop->release(); break;}
			if (qr.epr.lref==0 && qr.pack()!=RC_OK) continue;
			if (cur==~0u||(cmp=qr.epr.cmp(ops[cur].epr))>0) {cur=i; skp.epr=qs.epr=qr.epr; skip=&skp; nOK=1;}
			else if (cmp==0) {nOK++; flags|=qr.epr.flags&(PINEX_LOCKED|PINEX_XLOCKED|PINEX_ACL_CHKED|PINEX_EXTPID); qr.epr.flags|=flags;}
			else continue;
...
{
	qop.flags=QO_JOIN|QO_PIDSORT|QO_UNIQUE|QO_DEGREE|QO_STREAM; qop.nIn=nOps;
	for (ulong i=0; i<nOps; i++) {
...
RC MergeOp::next(PINEx &qr1,const PINEx *skip)
{
	PINEx qr2(ses); RC rc; bool fQ1=false,fQ2=false;
	if ((state&QST_INIT)!=0) {
	...
	for (qr1.cleanup(),qr1=saveID,qr1.epr=saveEPR;;) {
		if ((state&QOS_ADVN)!=0) {
			freeV(vals[0]); vals[0]=vals[2]; vals[2].setError(); qr1=nextR; state&=~QOS_ADVN;
		} else if ((state&QOS_EOF1)!=0) return RC_EOF;
		else if ((state&QOS_ADV1)!=0) {
			qr2.release(); queryOp2->release(); fQ1=true;
			freeV(vals[0]); vals[0].setError(propID1);
			if ((rc=queryOp->next(qr1,skip))!=RC_OK) {
				state=state&~QOS_ADV1|QOS_EOF1|QOS_EOF2; return rc;
			} else if ((state&QOS_EOF2)==0) {
				state&=~QOS_ADV1;
				if (propID1==PROP_SPEC_PINID) {
					if (qr1.id.pid==STORE_INVALID_PID && (rc=qr1.unpack())!=RC_OK) return rc;
					vals[0].set(qr1.id);
				} else if (queryOp->getData(qr1,&vals[0],1)!=RC_OK) {
					if (rc!=RC_DEADLOCK && rc!=RC_SHUTDOWN) {state|=QOS_ADV1; continue;}		// RC_NOACCESS || RC_DELETED || RC_NOTFOUND
					return rc;
				}
			}
			saveID=qr1.id; saveEPR=qr1.epr;
		}
		if ((state&QOS_EOF2)!=0) return op==QRY_EXCEPT?RC_OK:RC_EOF;
		if ((state&QOS_ADV2)!=0) {
			assert((state&QOS_EOF1)==0);
			qr1.release(); queryOp->release(); fQ2=true;
			freeV(vals[1]); vals[1].setError(propID2);
			if ((rc=queryOp2->next(qr2,skip))!=RC_OK) {
			...
			if (propID2==PROP_SPEC_PINID) {
				if (qr2.id.pid==STORE_INVALID_PID && (rc=qr2.unpack())!=RC_OK) return rc;
				vals[1].set(qr2.id);
			} else if (queryOp2->getData(qr2,&vals[1],1)!=RC_OK) {
		...
		if (cmp>0) state|=QOS_ADV2;
			...
			else {freeV(vals[0]); vals[0]=vals[2]; vals[2].setError(); qr1=nextR; state&=~QOS_NEXT;}
			...
			case 0:
				...
				if ((state&QOS_NEXT)==0) {
					...
					while ((rc=queryOp->next(nextR,skip))==RC_OK) {
						if (propID1==PROP_SPEC_PINID) {
							if (nextR.id.pid==STORE_INVALID_PID && (rc=nextR.unpack())!=RC_OK) return rc;
							vals[2].set(nextR.id); break;
						}
						if (queryOp->getData(nextR,&vals[2],1)==RC_OK) break;
			...
			if (op!=QRY_EXCEPT) {
				if (qr1.epr.lref!=0 && PINRef::isColl(qr1.epr.buf,qr1.epr.lref) && op<QRY_UNION && (mode&QO_UNIQUE)!=0) {
					if (pids!=NULL) {if ((*pids)[qr1]) continue;}
					else if ((pids=new(ses) PIDStore(ses))==NULL) return RC_NORESOURCES;
					if ((rc=((*pids)+=qr1))!=RC_OK) return rc;
					PINRef::changeFColl(qr1.epr.buf,qr1.epr.lref,false);
				}
				return RC_OK;
			}
		}
...
RC MergeOp::release()
{
	saveR.release(); nextR.release(); RC rc;
...
void MergeOp::getOpDescr(QODescr& qop)
{
	...
	qop.level=max(qop.level,qop2.level)+1; qop.nIn=2; qop.nOut=1;	// 2 if join and ...
...
RC HashOp::next(PINEx &qr,const PINEx *skip)
{
	RC rc=RC_OK;
	if ((state&QST_INIT)!=0) {
	...
	else do if ((rc=queryOp->next(qr,skip))==RC_OK) skip=NULL; else {state|=QST_EOF; break;} while (!(*pids)[qr]);
	return rc;
}
...
void HashOp::getOpDescr(QODescr& qop)
{
	...
	qop.level=max(qop.level,qop2.level)+1; qop.nIn=2; qop.nOut=1;
}
...
RC LoopJoin::next(PINEx& qr1,const PINEx *skip)
{
	PINEx qr2(ses); RC rc;
	if ((state&QST_INIT)!=0) {
		state&=~QST_INIT;
